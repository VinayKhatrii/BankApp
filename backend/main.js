...
module.exports = function (app) {
    ...
    app.post("/api/user/new", async (req, res) => {
        try {
            const details = req.body;
            const isDone = await saveDetails.saveNewAccount(details);
            if (isDone) {
                res.send({ success: true })
            } else {
                res.send({ success: false })
            ...
            console.error("Error in Api: /api/user/new" + err)
            res.status(500).send("Internal server error")
        }
    })

    app.post("/api/user/newApplications", async (req, res) => {
        try {
            const details = await saveDetails.getNewAccApplications();
            res.send(
                {
                    success: true,
                    details: details
            ...
            console.error("Error in Api: /api/user/newApplications" + err)
            res.status(500).send("Internal server error")
        }
    })
}
